...
class DataProcessor:
    ...
    def create_advanced_features(self, df_train: pd.DataFrame, df_test: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame | None]:
        ...
        # Cópias e flags
        df_train = df_train.copy().sort_values(['store', 'item', 'date']).reset_index(drop=True)
        df_train['_is_test'] = 0

        df_test_in = None
        test_had_sales = False
        if df_test is not None:
            df_test_in = df_test.copy().sort_values(['store', 'item', 'date']).reset_index(drop=True)
        ...
        # Lags - calculados apenas nos dados de treino
        if cfg.get('create_lags', True):
            # Primeiro calcula lags apenas nos dados de treino
            train_mask = df_all['_is_test'] == 0
            df_train_only = df_all[train_mask].copy()
            
            for lag in cfg.get('lag_periods', [1, 7, 30]):
                df_train_only[f'sales_lag_{lag}'] = df_train_only.groupby(['store', 'item'])['sales'].shift(lag)
            
            # Atualiza os valores de lag no DataFrame completo
            for lag in cfg.get('lag_periods', [1, 7, 30]):
                df_all[f'sales_lag_{lag}'] = np.nan
                df_all.loc[train_mask, f'sales_lag_{lag}'] = df_train_only[f'sales_lag_{lag}'].values

        # Rolling (sem usar o ponto atual) - IMPLEMENTAÇÃO CORRIGIDA
        if cfg.get('create_rolling_stats', True):
            windows = cfg.get('rolling_windows', [7, 14, 30])
            
            # Inicializar colunas de rolling
            for w in windows:
                df_all[f'sales_rolling_mean_{w}'] = np.nan
                df_all[f'sales_rolling_std_{w}'] = np.nan
            
            # Processar apenas dados de treino para cálculo de rolling
            train_mask = df_all['_is_test'] == 0
            train_groups = df_all[train_mask].groupby(['store', 'item'])
            
            for (store, item), train_group in train_groups:
                group_indices = train_group.index
                group_sales = train_group['sales'].tolist()
                
                for w in windows:
                    rolling_means = []
                    rolling_stds = []
                    
                    for i in range(len(group_sales)):
                        start_idx = max(0, i - w)
                        past_values = group_sales[start_idx:i]  # não inclui i
                        
                        if len(past_values) == 0:
                            rolling_means.append(np.nan)
                            rolling_stds.append(np.nan)
                        else:
                            rolling_means.append(float(np.mean(past_values)))
                            rolling_stds.append(float(np.std(past_values)) if len(past_values) > 1 else np.nan)
                    
                    # Atribuir de volta ao DataFrame
                    df_all.loc[group_indices, f'sales_rolling_mean_{w}'] = rolling_means
                    df_all.loc[group_indices, f'sales_rolling_std_{w}'] = rolling_stds
